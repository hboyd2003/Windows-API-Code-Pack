...
using HandlerNativeMethods = Microsoft.WindowsAPICodePack.ShellExtensions.Interop.HandlerNativeMethods;
...
/// <summary>
///     This is the base class for all preview handlers and provides their basic functionality. To create a custom preview
///     handler a class
///     must derive from this, use the <see cref="PreviewHandlerAttribute" />, and implement 1 or more of the following
///     interfaces:
    ...
    IOleWindow, IObjectWithSite, IInitializeWithStream, IInitializeWithItem, IInitializeWithFile
{
    ...
    void IInitializeWithItem.Initialize(object shellItem, AccessModes accessMode)
    {
        ...
        using (var shellObject = ShellObjectFactory.Create((IShellItem)shellItem))
        {
    ...
    private static void RegisterPreviewHandler(Guid previewerGuid, PreviewHandlerAttribute attribute)
    {
        ...
        using (var handlersKey =
               Registry.LocalMachine.OpenSubKey("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\PreviewHandlers", true))
        {
        ...
        using (var idKey = clsidKey.OpenSubKey(guid, true))
        {
            ...
            idKey.SetValue("DisableLowILProcessIsolation", attribute.DisableLowILProcessIsolation ? 1 : 0,
        ...
        foreach (var extension in attribute.Extensions.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            using (var previewKey =
                   shellexKey.CreateSubKey(Shell.Interop.HandlerNativeMethods.PreviewHandlerGuid.ToString("B")))
            {
