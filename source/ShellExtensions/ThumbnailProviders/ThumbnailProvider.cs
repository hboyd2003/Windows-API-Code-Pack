...
/// <summary>
///     This is the base class for all thumbnail providers and provides their basic functionality. To create a custom
///     thumbnail provider a
///     class must derive from this, use the <see cref="ThumbnailProviderAttribute" />, and implement 1 or more of the
    ...
    IInitializeWithStream, IInitializeWithItem, IInitializeWithFile
{
    ...
    /// <summary>
    ///     Sets the AlphaType of the generated thumbnail. Override this method in a derived class to change the thumbnails
    ///     AlphaType,
    ///     default is Unknown.
    /// </summary>
    ...
    {
        if (disposing && _stream != null) _stream.Dispose();
    }
    ...
    private static void RegisterThumbnailHandler(string guid, ThumbnailProviderAttribute attribute)
    {
        ...
        {
            guidKey.SetValue("DisableProcessIsolation", attribute.DisableProcessIsolation ? 1 : 0,
        ...
        var extensions = attribute.Extensions.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
        foreach (var extension in extensions)
            ...
            using (var providerKey =
                   shellExKey.CreateSubKey(Shell.Interop.HandlerNativeMethods.ThumbnailProviderGuid.ToString("B")))
            {
