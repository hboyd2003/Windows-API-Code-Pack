...
public class MappingPropertyBag : CriticalFinalizerObject, IDisposable
{
    ...
    internal MappingPropertyBag(MappingOptions options, string text)
    {
        ...
        if (!_serviceCache.RegisterResource()) throw new LinguisticException();
        _win32PropertyBag._size = InteropTools.SizeOfWin32PropertyBag;
        if (options != null) _options = InteropTools.Pack(ref options._win32Options);
    ...
    /// <summary>
    ///     Formats the low-level data contained in this <see cref="MappingPropertyBag">MappingPropertyBag</see> using an
    ///     implementation of
    ...
    /// <summary>
    ///     An array of <see cref="MappingDataRange">MappingDataRange</see> objects containing all recognized text range
    ///     results. This member
    ...
    {
        if (_win32PropertyBag._context == IntPtr.Zero) return true;
