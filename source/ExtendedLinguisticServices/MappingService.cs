...
/// <summary>
///     Represents an ELS service. A detailed overview of the Extended Linguistic Services platform is available at:
...
public class MappingService
{
    ...
    ///     If the service with the specified guid doesn't exist, a <see cref="LinguisticException">LinguisticException</see>
    ///     is thrown.
    /// </summary>
    ...
    public MappingService(Guid serviceIdentifier)
    {
        ...
        else // pService is IntPtr.Zero in this case.
        {
            ...
            try
            {
                ...
                if (hresult != 0) throw new LinguisticException(hresult);
                if (servicePointer == IntPtr.Zero) throw new InvalidOperationException();
                if (serviceCount != 1)
                {
    ...
    /// <summary>
    ///     Optional. A collection of input content types, following the format of the MIME content types, that identify the
    ///     format that the
    ///     service interprets when the application passes data. Examples of content types are "text/plain", "text/html" and
    ///     "text/css".
    /// </summary>
    public IEnumerable<string> InputContentTypes => InteropTools.UnpackStringArray(_win32Service._inputContentTypes,
        _win32Service._inputContentTypesCount);

    /// <summary>
    ///     A collection of the input languages, following the IETF naming convention, that the service accepts. This member is
    ///     set to null
    ///     if the service can work with any input language.
    /// </summary>
    public IEnumerable<string> InputLanguages =>
        InteropTools.UnpackStringArray(_win32Service._inputLanguages, _win32Service._inputLanguagesCount);

    /// <summary>
    ///     A collection of input scripts, with Unicode standard script names, that are supported by the service. This member
    ///     is set to null
    ...
    public IEnumerable<string> InputScripts =>
        InteropTools.UnpackStringArray(_win32Service._inputScripts, _win32Service._inputScriptsCount);
    ...
    /// <summary>
    ///     Optional. A collection of output content types, following the format of the MIME content types, that identify the
    ///     format in which
    ///     the service retrieves data.
    /// </summary>
    public IEnumerable<string> OutputContentTypes => InteropTools.UnpackStringArray(_win32Service._outputContentTypes,
        _win32Service._outputContentTypesCount);

    /// <summary>
    ///     A collection of output languages, following the IETF naming convention, in which the service can retrieve results.
    ///     This member is
    ...
    public IEnumerable<string> OutputLanguages =>
        InteropTools.UnpackStringArray(_win32Service._outputLanguages, _win32Service._outputLanguagesCount);
    ...
    public IEnumerable<string> OutputScripts =>
        InteropTools.UnpackStringArray(_win32Service._outputScripts, _win32Service._outputScriptsCount);
    ...
    /// <summary>
    ///     Flag indicating the language mapping between input language and output language that is supported by the service.
    ///     Possible values
    ///     are shown in the following table. 0 - The input and output languages are not paired and the service can receive
    ///     data in any of
    ///     the input languages and render data in any of the output languages. 1 - The arrays of the input and output
    ///     languages for the
    ///     service are paired. In other words, given a particular input language, the service retrieves results in the paired
    ///     language
    ///     defined in the output language array. Use of the language pairing can be useful, for example, in bilingual
    ...
    ///     A starting index inside the text recognition results for a recognized text range. This value should be between 0
    ///     and the range count.
    /// </param>
    ...
    public static void DoAction(MappingPropertyBag bag, int rangeIndex, string actionId)
    {
        ...
        if (rangeIndex < 0) throw new LinguisticException(LinguisticException.InvalidArgs);
    ...
    /// </summary>
    /// <param name="options">
    ///     Optional. A <see cref="MappingEnumOptions">MappingEnumOptions</see> object containing criteria to use during
    ///     enumeration of
    ///     services. The application specifies null for this parameter to retrieve all installed services.
    /// </param>
    /// <returns>
    ///     An array of <see cref="MappingService">MappingService</see> objects matching the criteria supplied in the options
    ///     parameter.
    /// </returns>
    public static MappingService[] GetServices(MappingEnumOptions options)
    {
        ...
        try
        {
            ...
            if (hresult != 0) throw new LinguisticException(hresult);
            if (servicePointer == IntPtr.Zero != (serviceCount == 0)) throw new InvalidOperationException();

            var services = new IntPtr[serviceCount];
            ServiceCache.Instance.RegisterServices(ref servicePointer, services);
            var result = new MappingService[serviceCount];
            for (var i = 0; i < serviceCount; ++i) result[i] = new MappingService(services[i]);
            return result;
        }
        finally
        {
            if (servicePointer != IntPtr.Zero)
                // Ignore the result if an exception is being thrown.
                Win32NativeMethods.MappingFreeServicesVoid(servicePointer);
            if (guidPointer != IntPtr.Zero) Marshal.FreeHGlobal(guidPointer);
        }
    }
    ...
    ///     An application callback delegate to receive callbacks with the results from the action operation. Cannot be set to
    ///     null.
    /// </param>
    /// <param name="callerData">
    ///     Optional. Private application object passed to the callback function by a service after the action operation is
    ///     complete. The
    ///     application must set this parameter to null to indicate no private application data.
    /// </param>
    /// <returns>
    ///     A <see cref="MappingActionAsyncResult">MappingActionAsyncResult</see> object describing the asynchronous
    ///     operation.
    /// </returns>
    public MappingActionAsyncResult BeginDoAction(MappingPropertyBag bag, int rangeIndex, string actionId,
        AsyncCallback asyncCallback, object callerData)
    {
    ...
    /// <summary>
    ///     Calls an ELS service to recognize text. For example, the Microsoft Language Detection service will attempt to
    ///     recognize the
    ///     language in which the input text is written. The execution will be handled in a thread from the managed thread
    ///     pool, and the
    ///     asynchronous wait handle of the returned <see cref="MappingRecognizeAsyncResult">MappingRecognizeAsyncResult</see>
    ///     object will be
    ...
    /// </summary>
    /// <param name="text">
    ///     The text to recognize. The text must be UTF-16, but some services have additional requirements for the input
    ///     format. This
    ///     parameter cannot be set to null.
    /// </param>
    /// <param name="options">
    ///     Optional. A <see cref="MappingOptions">MappingOptions</see> object containing options that affect the result and
    ///     behavior of text
    ///     recognition. The application does not have to specify values for all object members. This parameter can be set to
    ///     null to use the
    ///     default mapping options.
    /// </param>
    /// <param name="asyncCallback">
    ///     An application callback delegate to receive callbacks with the results from the recognize operation. Cannot be set
    ///     to null.
    /// </param>
    /// <param name="callerData">
    ///     Optional. Private application object passed to the callback function by a service after text recognition is
    ///     complete. The
    ///     application must set this parameter to null to indicate no private application data.
    /// </param>
    /// <returns>
    ///     A <see cref="MappingRecognizeAsyncResult">MappingRecognizeAsyncResult</see> object describing the asynchronous
    ///     operation.
    /// </returns>
    public MappingRecognizeAsyncResult BeginRecognizeText(string text, MappingOptions options,
        AsyncCallback asyncCallback, object callerData)
    {
    ...
    /// <param name="index">
    ///     Index inside the specified text to be used by the service. This value should be between 0 and length-1. If the
    ///     application wants
    ...
    ///     operation.
    /// </returns>
    public MappingRecognizeAsyncResult BeginRecognizeText(string text, int length, int index, MappingOptions options,
        AsyncCallback asyncCallback, object callerData)
    {
    ...
    /// </param>
    /// <returns>
    ///     A <see cref="MappingPropertyBag">MappingPropertyBag</see> object in which the service has stored its results. The
    ///     structure is
    ...
    {
        if (text == null) throw new ArgumentNullException("text");
        if (length > text.Length || length < 0) throw new ArgumentOutOfRangeException("length");
    ...
    {
        if (!RunningOnWin7) throw new PlatformNotSupportedException(LocalizedMessages.SupportedWindows7);
    }
