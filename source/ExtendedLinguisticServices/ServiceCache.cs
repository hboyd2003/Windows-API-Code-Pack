...
internal class ServiceCache : CriticalFinalizerObject
{
    ...
    internal static ServiceCache Instance { get; } = new();
    ...
    internal void RegisterServices(ref IntPtr originalPtr, IntPtr[] services)
    {
        ...
        {
            if (!succeeded) RollBack(originalPtr, length);
    ...
    {
        if (Interlocked.Decrement(ref _resourceRefCount) == 0 && IsInvalid) FreeAllServices();
    }
    ...
    private void FreeAllServices()
    {
        ...
        {
            foreach (var servicePtr in _servicePointers) Win32NativeMethods.MappingFreeServicesVoid(servicePtr);
    ...
    private void ReleaseHandle()
    {
        if (!IsInvalid)
        {
            if (Interlocked.Read(ref _resourceRefCount) == 0) FreeAllServices();
    ...
    private void TryRegisterServices(IntPtr originalPtr, IntPtr[] services, ref bool addedToCache)
    {
        ...
        for (var i = 0; i < services.Length; ++i)
        {
            ...
            Debug.Assert(cachedValue != IntPtr.Zero, "Cached value is NULL");
