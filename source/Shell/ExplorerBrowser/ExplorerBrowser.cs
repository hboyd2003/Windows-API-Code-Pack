    ...
    IMessageFilter
{
    ...
    public ShellObjectCollection Items
    {
        get
        {
            if (shellItemsArray != null) Marshal.ReleaseComObject(shellItemsArray);
    ...
    public ExplorerBrowserNavigationLog NavigationLog { get; }

    /// <summary>Options that control how the ExplorerBrowser navigates</summary>
    public ExplorerBrowserNavigationOptions NavigationOptions { get; }
    ...
    public string PropertyBagName
    {
        ...
        set
        {
            ...
            if (explorerBrowserControl != null) explorerBrowserControl.SetPropertyBag(propertyBagName);
        }
    }
    ...
    public ShellObjectCollection SelectedItems
    {
        get
        {
            if (selectedShellItemsArray != null) Marshal.ReleaseComObject(selectedShellItemsArray);
    ...
    {
        if (uChange == CommDlgBrowserStateChange.SelectionChange) FireSelectionChanged();
    ...
    /// <summary>
    ///     Fires when either a NavigationPending listener cancels the navigation, or if the operating system determines that
    ///     navigation is
    ///     not possible.
    /// </summary>
    ...
    /// <summary>
    ///     Fires when the item selected in the view has changed (i.e., a rename ). This is not the same as
    ///     SelectionChanged.
    /// </summary>
    public event EventHandler ViewSelectedItemChanged;

    /// <summary>
    ///     Clears the Explorer Browser of existing content, fills it with content from the specified container, and adds a new
    ///     point to the
    ///     Travel Log.
    /// </summary>
    ...
    {
        if (shellObject == null) throw new ArgumentNullException("shellObject");
    ...
    {
        if (ItemsChanged != null) ItemsChanged.Invoke(this, EventArgs.Empty);
    }

    internal void FireContentEnumerationComplete()
    {
        if (ViewEnumerationComplete != null) ViewEnumerationComplete.Invoke(this, EventArgs.Empty);
    }

    internal void FireSelectedItemChanged()
    {
        if (ViewSelectedItemChanged != null) ViewSelectedItemChanged.Invoke(this, EventArgs.Empty);
    }
