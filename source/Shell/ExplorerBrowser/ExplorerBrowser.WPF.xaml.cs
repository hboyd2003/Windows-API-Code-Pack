...
public partial class ExplorerBrowser : UserControl, IDisposable
{
    ...
    protected virtual void Dispose(bool disposed)
    {
        if (disposed)
        {
            if (itemsChanged != null) itemsChanged.Close();

            if (selectionChanged != null) selectionChanged.Close();
        }
    }
    ...
    private static void OnAdvancedQueryPaneChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (instance.ExplorerBrowserControl != null)
            instance.ExplorerBrowserControl.NavigationOptions.PaneVisibility.AdvancedQuery =
    ...
    private static void OnCommandsViewPaneChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        var instance = d as ExplorerBrowser;
        if (instance.ExplorerBrowserControl != null)
            instance.ExplorerBrowserControl.NavigationOptions.PaneVisibility.CommandsView =
                (PaneVisibilityState)e.NewValue;
    }
    ...
    private static void OnNavigationPaneChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        var instance = d as ExplorerBrowser;
        if (instance.ExplorerBrowserControl != null)
            instance.ExplorerBrowserControl.NavigationOptions.PaneVisibility.Navigation =
                (PaneVisibilityState)e.NewValue;
    }
    ...
    {
        navigationLog.Clear();
        foreach (var obj in ExplorerBrowserControl.NavigationLog.Locations) navigationLog.Add(obj);
    }
    ...
    private void UpdateDependencyPropertiesFromCLRPRoperties(object sender, EventArgs e)
    {
        ...
        {
            items.Clear();
            foreach (ShellObject obj in ExplorerBrowserControl.Items) items.Add(obj);
        }
        ...
        else if (selectionChangeWaitCount > 0)
        {
            ...
            {
                selectedItems.Clear();
                foreach (ShellObject obj in ExplorerBrowserControl.SelectedItems) selectedItems.Add(obj);
            }
        }
    }
}
