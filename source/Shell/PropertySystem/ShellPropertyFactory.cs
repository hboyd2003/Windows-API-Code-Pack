...
/// <summary>
///     Factory class for creating typed ShellProperties. Generates/caches expressions to create generic
///     ShellProperties.
/// </summary>
internal static class ShellPropertyFactory
{
    ...
    private static readonly Dictionary<int, Func<PropertyKey, ShellPropertyDescription, object, IShellProperty>>
        _storeCache
            = new();
    ...
    private static Func<PropertyKey, ShellPropertyDescription, object, IShellProperty> ExpressConstructor(Type type,
        Type[] argTypes)
    {
    ...
    {
        var hash = 0;
        foreach (var type in types) hash = hash * 31 + type.GetHashCode();
        return hash;
    }
}
