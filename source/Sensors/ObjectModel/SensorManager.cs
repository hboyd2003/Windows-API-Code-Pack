...
public static class SensorManager
{
    ...
    private static readonly Dictionary<Guid, SensorTypeData> guidToSensorDescr = new();

    private static readonly NativeISensorManager sensorManager = new NativeSensorManager();

    private static readonly nativeSensorManagerEventSink sensorManagerEventSink = new();

    /// <summary>.NET type -&gt; type GUID.</summary>
    private static readonly Dictionary<Type, Guid> sensorTypeToGuid = new();
    ...
    public static T GetSensorBySensorId<T>(Guid sensorId) where T : Sensor
    {
        ...
        if (hr == HResult.ElementNotFound) throw new SensorPlatformException(LocalizedMessages.SensorsNotFound);

        if (nativeSensor != null) return GetSensorWrapperInstance<T>(nativeSensor);
    ...
    public static void RequestPermission(IntPtr parentWindowHandle, bool modal, SensorList<Sensor> sensors)
    {
        ...
        foreach (var sensor in sensors) sensorCollection.Add(sensor.internalObject);

        sensorManager.RequestPermissions(parentWindowHandle, sensorCollection, modal);
    }

    internal static SensorList<S> nativeSensorCollectionToSensorCollection<S>(ISensorCollection nativeCollection)
        where S : Sensor
    {
    ...
    {
        if (SensorsChanged != null) SensorsChanged.Invoke(new SensorsChangedEventArgs(sensorId, change));
    }

    /// <summary>
    ///     Interrogates assemblies currently loaded into the AppDomain for classes deriving from <see cref="Sensor" />. Builds
    ///     data
...
internal class nativeSensorManagerEventSink : ISensorManagerEvents
{
    public void OnSensorEnter(ISensor nativeSensor, NativeSensorState state)
    {
        if (state == NativeSensorState.Ready)
        {
            ...
            if (hr == HResult.Ok) SensorManager.OnSensorsChanged(sensorId, SensorAvailabilityChange.Addition);
        }
    }
}
