...
public sealed class TaskDialog : IDialogControlHost, IDisposable
{
    ...
    public void Close()
    {
        if (!NativeDialogShowing) throw new InvalidOperationException(LocalizedMessages.TaskDialogCloseNonShowing);
    ...
    internal int RaiseClosingEvent(int id)
    {
        ...
        if (handler != null)
        {
            ...
            if (buttonClicked == TaskDialogStandardButtons.None)
            {
                ...
                if (customButton == null) throw new InvalidOperationException(LocalizedMessages.TaskDialogBadButtonId);
    ...
    internal void RaiseHyperlinkClickEvent(string link)
    {
        ...
        if (handler != null) handler(this, new TaskDialogHyperlinkClickedEventArgs(link));
    }
    ...
    private static TaskDialogNativeMethods.TaskDialogButton[] BuildButtonStructArray(
        List<TaskDialogButtonBase> controls)
    {
    ...
    private static int FindDefaultButtonId(List<TaskDialogButtonBase> controls)
    {
        ...
        if (defaults.Count > 1) throw new InvalidOperationException(LocalizedMessages.TaskDialogOnlyOneDefaultControl);
    ...
    private void ApplyControlConfiguration(NativeTaskDialogSettings settings)
    {
        ...
        if (progressBar != null)
        {
            if (progressBar.State == TaskDialogProgressBarState.Marquee)
                settings.NativeConfiguration.taskDialogFlags |=
                    TaskDialogNativeMethods.TaskDialogOptions.ShowMarqueeProgressBar;
            else
                settings.NativeConfiguration.taskDialogFlags |=
                    TaskDialogNativeMethods.TaskDialogOptions.ShowProgressBar;
        }
        ...
        if (buttons.Count > 0 || commandLinks.Count > 0)
        {
            ...
            if (commandLinks.Count > 0)
                settings.NativeConfiguration.taskDialogFlags |=
                    TaskDialogNativeMethods.TaskDialogOptions.UseCommandLinks;
        ...
        if (radioButtons.Count > 0)
        {
            ...
            if (defaultRadioButton == TaskDialogNativeMethods.NoDefaultButtonSpecified)
                settings.NativeConfiguration.taskDialogFlags |=
                    TaskDialogNativeMethods.TaskDialogOptions.NoDefaultRadioButton;
        }
    }
    ...
    private void ApplyGeneralNativeConfiguration(TaskDialogNativeMethods.TaskDialogConfiguration dialogConfig)
    {
        ...
        if (ownerWindow != IntPtr.Zero) dialogConfig.parentHandle = ownerWindow;
    ...
    private void ApplyOptionConfiguration(TaskDialogNativeMethods.TaskDialogConfiguration dialogConfig)
    {
        ...
        if (cancelable) options |= TaskDialogNativeMethods.TaskDialogOptions.AllowCancel;
        ...
        if (hyperlinksEnabled) options |= TaskDialogNativeMethods.TaskDialogOptions.EnableHyperlinks;
        if (detailsExpanded) options |= TaskDialogNativeMethods.TaskDialogOptions.ExpandedByDefault;
        if (Tick != null) options |= TaskDialogNativeMethods.TaskDialogOptions.UseCallbackTimer;
    ...
    private void CleanUp()
    {
        ...
        if (progressBar != null) progressBar.Reset();
        ...
        if (nativeDialog != null) nativeDialog.Dispose();
    }
