...
public static class PowerManager
{
    ...
    /// <summary>
    ///     Gets a value that indicates the remaining battery life (as a percentage of the full battery charge). This value is
    ///     in the range
    ...
    ///     The caller does not have sufficient privileges to set this
    ///     property.
    /// </exception>
    /// <remarks>
    ///     This information is typically used by applications that display information but do not require user interaction.
    ///     For example,
    ///     video playback applications.
    /// </remarks>
    /// <permission cref="T:System.Security.Permissions.SecurityPermission">
    ///     to set this property. Demand value: <see cref="F:System.Security.Permissions.SecurityAction.Demand" />; Named
    ///     Permission Sets: <b>FullTrust</b>.
    /// </permission>
    ...
    public static PowerSource PowerSource
    {
        get
        {
            ...
            if (IsUpsPresent) return PowerSource.Ups;

            if (!IsBatteryPresent || GetCurrentBatteryState().ACOnline) return PowerSource.AC;
    ...
    /// <summary>
    ///     Raised when the system will not be moving into an idle state in the near future so applications should perform any
    ///     tasks that
    ...
    /// <returns>
    ///     A <see cref="BatteryState" /> instance that represents the state of the battery at the time this method was
    ///     called.
    /// </returns>
    /// <exception cref="System.InvalidOperationException">The system does not have a battery.</exception>
    /// <exception cref="System.PlatformNotSupportedException">Requires XP/Windows Server 2003 or higher.</exception>
    ...
    /// <summary>
    ///     Allows an application to inform the system that it is in use, thereby preventing the system from entering the
    ///     sleeping power
    ...
    public static void SetThreadExecutionState(ExecutionStates executionStateOptions)
    {
        ...
        if (ret == ExecutionStates.None) throw new Win32Exception(LocalizedMessages.PowerExecutionStateFailed);
    }
}
