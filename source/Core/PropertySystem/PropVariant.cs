...
/// <remarks>
///     Originally sourced from http://blogs.msdn.com/adamroot/pages/interop-with-propvariants-in-net.aspx and modified to
///     support additional
...
public sealed class PropVariant : IDisposable
{
    ...
    [FieldOffset(8)] private readonly Blob _blob;

    [SuppressMessage("Microsoft.Reliability", "CA2006:UseSafeHandleToEncapsulateNativeResources")] [FieldOffset(8)]
    private IntPtr _ptr;
    ...
    [FieldOffset(8)] private readonly ulong _ulong;
    ...
    public PropVariant(DateTime[] value)
    {
        if (value == null) throw new ArgumentNullException("value");
        var fileTimeArr =
            new FILETIME[value.Length];

        for (var i = 0; i < value.Length; i++) fileTimeArr[i] = DateTimeToFileTime(value[i]);
    ...
    private Array GetVector<T>()
    {
        ...
        lock (_padlock)
        {
            if (_vectorActions == null) _vectorActions = GenerateVectorActions();
        }
        ...
        for (uint i = 0; i < count; i++) action(this, array, i);
    ...
    private static Array CrackSingleDimSafeArray(IntPtr psa)
    {
        ...
        for (var i = lBound; i <= uBound; ++i) array[i] = PropVariantNativeMethods.SafeArrayGetElement(psa, ref i);
