...
internal static class TaskDialogNativeMethods
{
    ...
    internal enum TaskDialogMessages
    {
        ...
        SetProgressBarMarquee =
            CoreNativeMethods.UserMessage +
            107, // wParam = 0 (stop marquee), wParam != 0 (start marquee), lparam = speed (milliseconds between repaints)

        SetElementText =
            CoreNativeMethods.UserMessage +
            108, // wParam = element (TASKDIALOG_ELEMENTS), lParam = new element text (LPCWSTR)
        ClickRadioButton = CoreNativeMethods.UserMessage + 110, // wParam = Radio Button ID

        EnableButton =
            CoreNativeMethods.UserMessage + 111, // lParam = 0 (disable), lParam != 0 (enable), wParam = Button ID

        EnableRadioButton =
            CoreNativeMethods.UserMessage + 112, // lParam = 0 (disable), lParam != 0 (enable), wParam = Radio Button ID

        ClickVerification =
            CoreNativeMethods.UserMessage + 113, // wParam = 0 (unchecked), 1 (checked), lParam = 1 (set key focus)

        UpdateElementText =
            CoreNativeMethods.UserMessage +
            114, // wParam = element (TASKDIALOG_ELEMENTS), lParam = new element text (LPCWSTR)
        ...
        UpdateIcon =
            CoreNativeMethods.UserMessage +
            116 // wParam = icon element (TASKDIALOG_ICON_ELEMENTS), lParam = new icon (hIcon if TDF_USE_HICON_* was set, PCWSTR otherwise)
    }
    ...
    internal struct TaskDialogButton
    {
        ...
        [MarshalAs(UnmanagedType.LPWStr)] internal string buttonText;
    }
    ...
    internal class TaskDialogConfiguration
    {
        ...
        [MarshalAs(UnmanagedType.LPWStr)] internal string windowTitle;

        internal IconUnion mainIcon; // NOTE: 32-bit union field, holds pszMainIcon as well

        [MarshalAs(UnmanagedType.LPWStr)] internal string mainInstruction;

        [MarshalAs(UnmanagedType.LPWStr)] internal string content;
        ...
        [MarshalAs(UnmanagedType.LPWStr)] internal string verificationText;

        [MarshalAs(UnmanagedType.LPWStr)] internal string expandedInformation;

        [MarshalAs(UnmanagedType.LPWStr)] internal string expandedControlText;

        [MarshalAs(UnmanagedType.LPWStr)] internal string collapsedControlText;

        internal IconUnion footerIcon; // NOTE: 32-bit union field, holds pszFooterIcon as well

        [MarshalAs(UnmanagedType.LPWStr)] internal string footerText;
