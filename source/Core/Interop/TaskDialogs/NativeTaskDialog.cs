...
/// <summary>
///     Encapsulates the native logic required to create, configure, and show a TaskDialog, via the TaskDialogIndirect()
///     Win32 function.
/// </summary>
/// <remarks>
///     A new instance of this class should be created for each messagebox show, as the HWNDs for TaskDialogs do not remain
///     constant across
...
internal class NativeTaskDialog : IDisposable
{
    ...
    private readonly IntPtr[] updatedStrings =
        new IntPtr[Enum.GetNames(typeof(TaskDialogNativeMethods.TaskDialogElements)).Length];
    ...
    internal void NativeShow()
    {
        ...
        if (settings == null) throw new InvalidOperationException(LocalizedMessages.NativeTaskDialogConfigurationError);
    ...
    protected void Dispose(bool disposing)
    {
        if (!disposed)
        {
            ...
            if (ShowState == DialogShowState.Showing) NativeClose(TaskDialogResult.Cancel);
    ...
    private int HandleButtonClick(int id)
    {
        ...
        if (ShowState != DialogShowState.Closing) outerDialog.RaiseButtonClickEvent(id);
        ...
        if (id < DialogsDefaults.MinimumDialogControlId) return outerDialog.RaiseClosingEvent(id);
